...
export default function ExchangeChatPage() {
  ...
  const receiverId = "64df1b1234567890abcdef12"; // Replace with actual receiver ID (e.g., Alice's)

  useEffect(() => {
    fetch(`http://localhost:5000/api/exchangechat/${senderId}/${receiverId}`)
      .then((res) => res.json())
      .then((data) => {
        const formatted = data.map((m) => ({
          ...
          sender: m.senderId === senderId ? "self" : "other",
        }));
  ...
  const sendMessage = () => {
    ...
    fetch("http://localhost:5000/api/exchangechat", {
      method: "POST",
      ...
      .then((res) => res.json())
      .then((savedMsg) => {
        const time = new Date(savedMsg.timestamp).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });
        setMessages([...messages, { text: savedMsg.message, time, sender: "self" }]);
        setNewMessage("");
      })
